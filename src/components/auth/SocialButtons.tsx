...
const BUTTONS: { provider: Provider; label: string; bg: string; color: string; border: string; logo: React.ReactNode }[] = [
  {
    provider: "google",
    label: "Google로 계속하기",
    bg: "#fff",
...
export function SocialButtons() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {BUTTONS.map(({ provider, label, bg, color, border, logo }) => (
        <button
          key={provider}
          onClick={() => signInWith(provider)}
          style={{
            height: 50, borderRadius: 14, border: `1.5px solid ${border}`,
