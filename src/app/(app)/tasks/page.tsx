...
const T = {
  ko: { currentStage: "현재 단계", inProgress: "진행 중", done: "완료", next: "다음", urgent: "긴급", doneCount: (n: number) => `${n}개 완료`, remaining: (n: number) => `${n}개 남음`, steps: "진행 단계", howTo: "어떻게 하나요?", complete: "완료 표시", undo: "취소", nearbyTitle: "근처 관련 장소" },
  en: { currentStage: "Current Stage", inProgress: "In Progress", done: "Done", next: "Up Next", urgent: "Urgent", doneCount: (n: number) => `${n} done`, remaining: (n: number) => `${n} remaining`, steps: "Steps", howTo: "How to do it", complete: "Mark Complete", undo: "Undo", nearbyTitle: "Nearby Places" },
};
...
export default function TasksPage() {
  ...
  const selectedTask = TASKS.find((t) => t.id === selectedId) ?? TASKS[2];
        ...
        return (
          <div
            ...
            style={{
              ...
              padding: "12px", marginBottom: 7,
              background: isSelected ? "var(--card-selected)" : "var(--card)",
              borderRadius: 12,
              border: isSelected ? "1.5px solid #15b6c1" : task.urgent ? "1.5px solid #FFCDD2" : "1px solid var(--border)",
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 2, flexWrap: "wrap" }}>
                ...
                {task.urgent && !isChecked && <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 4, background: "#FFEBEE", color: "#C62828", fontWeight: 700, flexShrink: 0 }}>{task.deadline} · {t.urgent}</span>}
                ...
                {task.next && !isChecked && <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 4, background: "#E8F4FF", color: "#1565C0", fontWeight: 700, flexShrink: 0 }}>{t.next}</span>}
              </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 28 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              {selectedTask.urgent && !isChecked && <span style={{ fontSize: 10, padding: "3px 9px", borderRadius: 6, background: "#FFEBEE", color: "#C62828", fontWeight: 700 }}>{selectedTask.deadline} · {t.urgent}</span>}
              ...
              {selectedTask.next && !isChecked && <span style={{ fontSize: 10, padding: "3px 9px", borderRadius: 6, background: "#E8F4FF", color: "#1565C0", fontWeight: 700 }}>{t.next}</span>}
            </div>
